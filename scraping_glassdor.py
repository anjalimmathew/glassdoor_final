...
SCRAPFLY = ScrapflyClient(key="PUT KEY HERE")

global added_reviews, add_central


added_reviews = pd.read_excel('new_reviews.xlsx')
add_central = pd.DataFrame(columns=['Company Name', 'Total Reviews', 'Reviews Extracted'])
...
async def scrape_reviews(url: str, max_pages: Optional[int] = None) -> Dict:
    ...
    other_pages = [
        ...
        for page in range(4, total_pages + 1) #CHANGE THIS --> want to detect pages manually
    ]
    async for result in SCRAPFLY.concurrent_scrape(other_pages):
        if not isinstance(result, ScrapflyScrapeError):
...
async def processRow(row):

    coName = row['company_name']
    ebay_jobs_in_US = await scrape_reviews(
        url=row['review_url'],
        max_pages=10 
    )
    print('done scraping', coName)
    
    out_file = open('page.json', 'w')
    json.dump(ebay_jobs_in_US, out_file, indent=2)
    out_file.close()
    ...
    reviews['COMPANY NAME'] = coName
    global added_reviews, add_central
    added_reviews = pd.concat([added_reviews, reviews], ignore_index=True)
    
    add_central.loc[len(add_central), add_central.columns] = coName, data['filteredReviewsCount'], num_reviews
...
async def run():
    companies_to_parse = pd.read_excel('mini_firms.xlsx')
    for _, row in companies_to_parse.iterrows():
        try:
            await processRow(row)
        except RuntimeError: 
            print('runtime error, appending 0s')
            add_central.loc[len(add_central), add_central.columns] = row['company_name'], 0, 0
            continue
        except AttributeError:
            print('normal out of bounds error')
            break
    ...
    added_reviews.to_excel('new_reviews.xlsx')
    add_central.to_excel('new_central_temp.xlsx')
